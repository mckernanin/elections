...
if ( is_page( 'request-an-election' ) && is_user_logged_in() ) {
	...
	if ( isset( $_GET['update'] ) ) {
		...
		echo '<h3>' . sprintf( esc_html( 'Thank you%s, your election has been updated. You will be notified by email once your election has been scheduled.', 'OA-Election' ), esc_html( $name ) ) . '</h3><br />';
	}
	...
	if ( isset( $_GET['new_election'] ) ) {
		...
		$name = $name ? ' ' . $name : '';
		// Add notice of submission to our output
		echo '<h3>' . sprintf( esc_html( 'Thank you%s, your election has been submitted. You will be notified by email once your election has been scheduled.', 'OA-Election' ), esc_html( $name ) ) . '</h3><br />';
	}

	echo '<h2>' . esc_html( $election->meta_box['title'] ) . '</h2>';
	echo cmb2_get_metabox_form( $election, 'unit_fields', $metabox_form_options );
}
